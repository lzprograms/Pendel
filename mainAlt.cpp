...
class Pendel {
    ...
    public:
        Pendel(){
            ...
            axis = new Axis(chip, 23, 24, 22, 4, 450, 0.2);
        }
...
int main() {
    ...
    const double Kp = -1.0, Kd = 0, Kx = 0; // Kx sorgt fürs Zentrieren
    const double xMin = 0, 
    ...
    for (int i = 0; i < 50000; ++i) {
        ...
        if(170 < winkel && winkel < 190){
            ...
            double pos = p.getPos();
            move = Kp * abs(sollWinkel-winkel)*(sollWinkel-winkel) + Kd * winkelRate - Kx * (pos - xMitte);
